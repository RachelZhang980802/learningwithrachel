...
const articles: Article[] = [
  { number: "01", title: "I Become a Student", author: "Lincoln Steffens", theme: "Education · Curiosity · Inquiry", image: "/media/covers/01-student.webp" },
  { number: "02", title: "Should the Robots Be Taxed?", author: "Kumar Sachidanandam", theme: "Justice · Property · Society", image: "/media/covers/02-taxed.webp" },
  { number: "03", title: "Roommates", author: "Max Apple", theme: "Kinship · Memory · Home", image: "/media/covers/03-roommates.webp" },
  { number: "04", title: "Never Trust Appearances", author: "Author forthcoming", theme: "Perception · Doubt · Identity", image: "/media/covers/04-appearances.webp" },
  { number: "05", title: "Courage", author: "Lydia Minatoya", theme: "Resolve · Voice · Becoming", image: "/media/covers/05-courage.webp" },
  { number: "06", title: "Reading", author: "Benjamin Franklin", theme: "Books · Discipline · Discovery", image: "/media/covers/06-reading.webp" },
  { number: "07", title: "What We Gain or Lose in Cities", author: "Jared Diamond", theme: "Cities · Exchange · Belonging", image: "/media/covers/07-cities.webp" },
  { number: "08", title: "The Guide to Being a Foodie Without Being Culturally Appropriative", author: "Rachel Kuo", theme: "Food · Culture · Respect", image: "/media/covers/08-foodie.webp" },
  { number: "09", title: "Stop Telling Me to Travel like a Local, Okay?", author: "Mari Uyehara", theme: "Travel · Place · Perspective", image: "/media/covers/09-travel.webp" },
  { number: "10", title: "Two Ways of Seeing a River", author: "Mark Twain", theme: "River · Knowledge · Wonder", image: "/media/covers/10-river.webp" },
];
...
function Header({ view, onNavigate }: { view: View; onNavigate: (view: View) => void }) {
  return (
    <header className="site-header">
      <button className="wordmark" onClick={() => onNavigate("library")} aria-label="NegentropyLearning home">
        <span className="ink-mark" aria-hidden="true">
          <img src="/media/brand/negentropy-learning-mark-v1.png" alt="" />
        </span>
      ...
      <nav aria-label="Global navigation">
        {(["library", "guide", "index", "about"] as View[]).map((item) => (
          <button key={item} className={view === item ? "is-current" : ""} onClick={() => onNavigate(item)}>
            {item === "guide" ? "Reading Guide" : item[0].toUpperCase() + item.slice(1)}
          </button>
        ))}
      </nav>
    </header>
...
export default function Home() {
  ...
  if (view !== "library") {
    return <SimplePage view={view} onNavigate={navigate} onRead={openReading} />;
  }
  ...
  return (
    <main className="library-page">
      ...
      <section className="hero" aria-labelledby="library-title">
        <h1 id="library-title">Read between<br />the lines.</h1>
      ...
      <footer className="library-footer"><span>AN ANNOTATED DIGITAL EDITION</span><span>EST. MMXXVI</span></footer>
    </main>
  );
}

function SimplePage({ view, onNavigate, onRead }: { view: Exclude<View, "library" | "reading">; onNavigate: (view: View) => void; onRead: (index: number) => void }) {
...
function ReadingPage({ article, index, showMuckRakeIntro, onOpenMuckRakeIntro, onDismissMuckRakeIntro, onBack, onPrevious, onNext }: { article: Article; index: number; showMuckRakeIntro: boolean; onOpenMuckRakeIntro: () => void; onDismissMuckRakeIntro: () => void; onBack: () => void; onPrevious?: () => void; onNext?: () => void }) {
  ...
  const navigation = (active: "text" | "analysis" | "quizzes" | "notes") => <nav className="reading-nav" aria-label="Article navigation">
    ...
    <button className={`reading-nav-tab${active === "notes" ? " is-active" : ""}`} aria-pressed={active === "notes"} onClick={() => { setAnalysisMode(false); setQuizOpen(false); setNotesOpen(value => !value); }}>Notes</button>
...
function SentenceStudio({ onExit, articleId, initialVocabulary }: { onExit: () => void; articleId: string; initialVocabulary?: VocabularyTarget }) {
  ...
  if (isStudent) return <ArticleOneNotebook sentences={sentences} selected={selected} paragraph={activeParagraph} ranges={paragraphRanges} panel={activePanel} onPanel={setActivePanel} onSelect={chooseSentence} onStep={stepAnalysisSentence} onExit={onExit} showClassificationBadge suppressClassificationLabel>
    {activePanel === "words" ? <ArticleOneWords key={selected} initialSourceRow={initialVocabulary?.sentence === selected + 1 ? initialVocabulary.sourceRow : undefined} sentence={selected + 1} text={sentences[selected]} contextAdornment={<SentenceBadge sentence={selected + 1} compact hideClassificationLabel/>}/>
      : activePanel === "structure" ? <SentenceStructure key={`01-${selected}`} sentence={sentences[selected]} articleId="01" notebook/>
      : activePanel && <div className="student-existing-content">
        <nav className="student-subtabs" aria-label="Activities">{(["meaning", "translation", "voice", "context"].includes(activePanel) ? studioPanels.filter((item) => item.id === "translation") : practicePanels.filter((item) => item.id !== "translation")).map((item) => <button key={item.id} aria-pressed={item.id === activePanel} onClick={() => setActivePanel(item.id)}>{item.label}</button>)}</nav>
        {activePanel === "translation" ? <p className="student-sentence-translation" lang="zh-CN">{article01Translations[sentences[selected]]}</p>
          : activePanel === "paraphrase" ? <ArticleOneParaphrase key={selected} sentence={sentences[selected]}/>
          : activePanel === "reuse" ? <ArticleOneReuse key={selected} sentence={sentences[selected]}/>
          : analysisReady ? <AnalysisContent panel={activePanel} depth={structureDepth} setDepth={setStructureDepth}/>
